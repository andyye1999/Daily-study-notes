...
[melgan]([(41 封私信 / 7 条消息) MelGan - 搜索结果 - 知乎 (zhihu.com)](https://www.zhihu.com/search?type=content&q=MelGan))   讲解判别器结果，多尺度 连续下采样后输出feature 和 score 
...
SEANet结合对抗性和重建损失，同时训练生成器和鉴别器。对抗性损失是在多个分辨率和时间上平均的铰链损失。更正式地说，令k∈{1,...,K}索引不同分辨率的各个鉴别器，并且t索引输出长度，即鉴别器k的logits数量Tk。 鉴别器损失可以写成：
